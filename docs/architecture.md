...
### 86. 打印平台多对象排布锁定与重新寻优架构（已实现）

- `createPrintPlatformMultiObjectLockedRotationLayoutPlan(...)` 在第 85 阶段协议之上接收稳定对象 ID 锁定集合；空集合直接复用原旋转寻优结果，非空集合把锁定状态写入方案身份，保证锁定、解锁会使旧预览自然失效。
- 锁定对象先转换为零位移、零旋转变化的 `locked` placement，并通过 `fitsInside` 与矩形安全间距关系完成越界和冲突校验。失败返回 `unplaceable`、空 `placements` 和对象级中文原因。
- 未锁定对象使用锁定边界及已放置边界的最大 X/Z 加安全间距生成稳定候选坐标，枚举当前/增加 90 度两种精确占地；状态按对象、目标坐标和朝向去重并限制为 8192 个，按行数、整体面积、总位移、旋转数和稳定签名排序。
- 新方案输出 `lockedObjectCount`、`adjustableObjectCount` 和逐对象 `locked` 标记。React 使用本地字符串数组保存会话锁定集合，来源变化时清除；间距和有效区域变化只改变方案身份，不写 Store。
- 三维叠加对锁定对象使用灰色目标边界和方向线，对可调整对象继续使用青色/橙色；DOM 暴露 `data-print-platform-layout-lock` 与 `data-print-platform-layout-locked`。确认过滤锁定对象和无变化对象后复用 `applyObjectPresentationBatch(...)`。
...
### 89. 打印平台多对象固定净间距分布架构（已实现）

- `src/model/printPlatformFixedGapLayout.ts` 是独立于 React、Three.js 和 Zustand 的纯计划协议。`createPrintPlatformFixedGapPlan(...)` 接收联合占地、有效区域、安全间距、目标净间距、锁定集合、已选集合和 X/Z 操作，输出稳定 `sourceIdentity`、空间顺序、逐对象当前/目标边界、中心、位移、相邻净间距、状态与整批 `canApply`。
- 输入层把目标净间距与碰撞安全间距保持为两个独立参数；目标净间距必须是非负有限数且不得小于安全间距。协议在目标轴按当前中心稳定排序，中心平局时按 `sourceIdentity`、`objectId` 排序，避免组件输入顺序影响结果。
- 空间顺序第一个对象标记为固定锚点并保持中心不动；后续对象使用上一对象目标最大边界、目标净间距和当前对象半尺寸递推目标中心。协议只修改一个水平轴，不生成 Y、旋转、缩放、颜色或几何变更。
- 校验层保留未选与锁定对象的当前边界并让其参与约束；目标边界必须完整位于安全有效区域，所有至少包含一个已选对象的对象对都要满足无水平重叠及当前安全间距。任一已选目标非法时计划不可确认，没有实际位移时禁止空提交。
- `ModelViewport` 以本地状态保存全中文目标净间距输入和固定净间距预览请求；来源、选择、锁定、安全间距、目标净间距、有效区域和操作写入方案身份。固定净间距与自动排布、手工拖动、对齐预览互斥，旧身份无法应用。
- `PrintPlatformOverlayLayer` 复用幽灵矩形与位移线呈现：首对象黄色、合法目标青色、非法目标红色，并通过中文 DOM 展示空间序号、相邻净间距、目标中心、位移和状态。多对象预览存在时图例始终启用 `has-scrollable-layout`，保证预览创建前输入与操作按钮也可达。
- 确认路径只提交 `selected && moved && status === 'valid'` 的对象，基于当前展示状态叠加目标轴位移并复用 `applyObjectPresentationBatch(...)` 原子创建一个中文版本；取消只清理组件会话，整批撤销重做沿用既有版本协议。
- `src/model/printPlatformFixedGapLayout.test.ts` 新增 10 个纯协议测试；阶段针对性测试 4 个文件 44/44，完整回归 36 个测试文件 389/389，TypeScript/Vite 构建与差异检查通过。浏览器覆盖默认/非法输入、合法预览、取消、确认、版本、撤销重做、锁定对象和面板滚动可达性，Console 为 0 个错误、0 个警告。

### 90. 打印平台多对象固定净间距锚点模式架构（已实现）

- `PrintPlatformFixedGapAnchorMode` 定义 `keep-first` 与 `keep-last` 两种显式模式，`PrintPlatformFixedGapPlan.anchorMode` 向视口和提交路径暴露最终语义；`createPrintPlatformFixedGapPlan(...)` 的末参数带 `keep-first` 默认值，保证既有调用兼容。
- `createSourceIdentity(...)` 把锚点模式写入稳定方案身份，使同一来源、选择、间距和操作下的首端与末端计划无法互相冒用。非法输入、选择不足和锁定目标的失败计划同样保留锚点模式。
- 协议先按目标轴当前中心和稳定身份生成唯一空间顺序，再标记首端或末端固定锚点。首端循环从索引 1 向后使用前一目标最大边界递推；末端循环从倒数第二项向前使用下一目标最小边界反向递推。
- 目标计算完成后统一按空间顺序重新计算 `previousGapMm`，因此首端和末端计划的对象明细使用同一“与前一对象净间距”语义；后续有效区域、对象对重叠、安全间距、空变化和批量提交逻辑无需分叉。
- `ModelViewport` 用本地 `printPlatformFixedGapAnchorMode` 保存中文选择，并在计划计算和创建请求时显式传入协议；选择变化清除预览，活动预览期间禁用锚点与间距输入。标题、摘要、明细、图例和中文版本描述都读取计划中的锚点模式，不依赖易漂移的当前控件值。
- 样式新增与毫米输入一致的锚点下拉框，继续复用内部滚动面板。纯协议测试累计 14 个，阶段针对性测试 4 个文件 48/48、完整回归 36 个文件 393/393；浏览器验证两端递推、版本与面板可达性，Console 为 0 个错误、0 个警告。

### 91. 打印平台多对象固定净间距任意对象锚点架构（已实现）

- `PrintPlatformFixedGapAnchorMode` 扩展为 `keep-first`、`keep-last` 和 `keep-selected`。计划新增 `anchorObjectId`，协议参数允许传入显式候选，并始终把最终解析后的真实锚点 ID 暴露给视口、图例和版本描述。
- 协议先按目标轴当前中心生成稳定空间顺序，中心平局继续使用 `sourceIdentity + objectId`；首端和末端直接解析空间首尾，指定对象模式只接受已选集合中的 ID，否则回退到按稳定身份排序的首个已选对象。
- `createSourceIdentity(...)` 同时写入锚点模式和最终锚点对象 ID。即使调用方传入失效 ID，计划身份也只记录真实解析结果，避免同一候选集合在不同输入顺序下产生漂移身份。
- 三种模式统一为一个 `anchorIndex` 双向递推算法：锚点前方从右向左使用下一目标最小边界计算中心，锚点后方从左向右使用上一目标最大边界计算中心；完成后统一计算 `previousGapMm`，边界、重叠、安全间距和原子应用语义不分叉。
- `ModelViewport` 增加指定锚点本地状态和稳定解析值。第二个中文下拉框只在“指定对象不动”时显示，候选复用当前已选未锁定对象；选择或锁定变化会清除预览并清空已失效的显式 ID，计划 memo 与创建请求都传入同一解析值。
- 活动预览期间锚点模式和锚点对象输入同时禁用；摘要、黄色幽灵锚点和版本描述读取计划内的最终对象名称，例如 `X 轴固定净间距分布（指定“上盖”不动）2 个打印对象`，不依赖预览后可能变化的控件状态。
- 纯协议累计 18 个测试，新增内部锚点 X/Z 双向递推、身份稳定和失效回退覆盖；阶段针对性测试 4 个文件 52/52，完整回归 36 个文件 397/397，TypeScript/Vite 构建与差异检查通过。浏览器当前两对象 Demo 覆盖候选、指定首末对象、版本与面板可达性，内部三对象语义由纯协议测试验证，Console 为 0 个错误、0 个警告。

**下一阶段架构方向：**增加独立的双端约束模式：同时锁定稳定空间顺序首尾对象，先根据固定端点中心跨度、各对象目标轴尺寸和对象数量计算唯一可实现平均净间距；只有输入目标在毫米容差内一致时才生成内部对象目标，冲突则返回全中文不可应用计划，继续复用统一边界、碰撞和原子提交层。
