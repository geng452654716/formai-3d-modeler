...
1. 前端只接受当前修订中已经完成 OpenCascade 精确解析的单个稳定面或一条种子稳定边。稳定平面可执行圆形/矩形凸台、圆孔/矩形孔/槽孔、整面向外拉伸或向内偏移；稳定平面所属种子边可执行单边、唯一切线连续边链或唯一平面边界 Wire 的整圈圆角/倒角。非平面稳定面只允许受限圆形凸台、圆孔、矩形凸台、矩形孔和槽孔，其所属种子边可执行单边或唯一切线连续边链圆角/倒角，但不允许整圈。
