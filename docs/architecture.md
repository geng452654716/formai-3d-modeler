...
稳定 CAD 平面局部特征第一版的数据流如下：

1. 前端只接受 `selectionMode=click`、单个 `geometryType=PLANE` 的稳定面上下文，并用确定性中文解析器生成圆形凸台或圆孔请求；请求携带清单修订号、零件 ID、稳定面 ID、原始毫米点击坐标、点击法线、半径、深度和原始指令。
2. Tauri 命令 `run_local_cad_feature` 与 Vite 本机路由 `/api/model/local-cad-feature` 执行相同的操作白名单、字符串长度、有限数值和尺寸范围校验，再调用 `modeling/local_cad_feature.py`。
3. Worker 读取当前 `generation-result.json`，拒绝过期修订号，并在当前 STEP 中使用 `partId + stableFaceId` 重新定位 OpenCascade 面。实际布尔方向只来自重新定位面的外法线；视口法线只用于点积一致性检查，点击点还要通过面距离校验。
4. 结果必须是有效、封闭、单一 Solid；凸台体积必须增加，圆孔体积必须减少；重新导出的 STL 必须能再次读入且体积误差在容差内。
5. 成功后重新生成稳定面描述、几何签名匹配摘要、专用选择 STL、面映射、用户 STL/STEP 和通用多零件 3MF，并原子更新清单修订号与 `localFeatures[]`。每条记录保存 `targetFace` 几何签名快照、创建修订号和重放状态；前端清除旧选择，因为原 `triangleIndex` 已失效。

Codex 局部计划链路只接受受限 JSON：整模参数变化与局部特征互斥；局部特征只能是 `add-cylinder` 或 `cut-cylinder`；后端与前端都会验证当前选择必须是点击单平面，并强制计划的 `partId + stableFaceId` 与 `faces[0] + hit` 一致。Codex 不具备任意脚本、CadQuery 或 shell 执行权限，最终中心和外法线也不会采用模型自行生成的坐标。

参数化整模重建先调用 `build_body()` / `build_cover()` 得到不含局部特征的基础实体，再按 `localFeatures[]` 历史顺序重放。每条记录执行前都要重新校验操作、零件、稳定面、记录中心、外法线、半径和深度；直接稳定 ID 不可用时，可用 `targetFace` 几何签名快照进行第一版辅助匹配。任一条失败都会在导出前终止，因此不会覆盖最后有效的 STEP、STL、3MF、选择网格或清单，也不允许静默跳过。

能力边界必须明确：稳定面 ID 和 `targetFace` 回退都是“几何签名匹配第一版”，不是 OpenCascade 原生永久拓扑命名；大幅拓扑变化、对称面或布尔重建可能重新编号。`triangleIndex` 只对同一次生成的选择 STL 有效，局部修改、重新三角化、第三方修复或普通导出都可能改变顺序。上传 STL 使用独立的网格区域和壁厚采样协议，不套用参数化 CAD 的稳定面映射。第一版仅支持点击单平面上的圆形凸台和圆孔，尚未提供框选多面布尔、曲面 UV、矩形孔/槽孔、整面拉伸/偏移、局部圆角/倒角、任意拓扑顶点/边/面编辑或通用自由曲面直接修改。
...
当前已经落地版本列表、任意版本恢复、分支截断、参数与通用开孔元数据对比，以及历史/当前 STL 的蓝橙重叠和按真实毫米宽度并排显示。桌面端还可按稳定零件 ID 读取历史与当前 STEP，执行 `current - base` 和 `base - current` 两个 OpenCascade 精确布尔差集；视口以绿色显示新增区域、红色显示删除区域，并报告体积。该实体级差异不依赖面 ID。参数化 CAD 清单现已保存面几何签名与近似稳定 ID，版本实体对比可按“零件 ID + 面稳定 ID”统计共享、新增和消失编号；当前选择 STL 已能把同次生成的三角面命中回映射到稳定面，点击单个稳定平面也已能转换为圆形凸台或圆孔的可验证 OpenCascade 局部特征，并在参数化整模重建时安全重放。仍未实现的是任意拓扑修改下的永久稳定命名、完整的局部特征类型和曲面/多面操作。
