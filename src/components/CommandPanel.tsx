...
interface CommandPanelProps {
  command: string;
  generatedDiagnosticBlocks: readonly string[];
...
export function CommandPanel({ command, generatedDiagnosticBlocks, onCommandChange }: CommandPanelProps) {
  ...
  const removeDiagnosticDraftBlock = () => {
    const result = removeMeshPlanarRegionCodexAnalysisDraftBlock(command, generatedDiagnosticBlocks);
    if (result.status !== 'removed') {
      setDiagnosticDraftNotice('诊断块不完整或存在歧义，为保护您的文字，未执行移除。');
      return;
    }
    onCommandChange(result.draft);
    setDiagnosticDraftNotice('已移除诊断块，剩余建模指令尚未执行。');
  };

  const updateDraft = (nextDraft: string) => {
    setDiagnosticDraftNotice(null);
    onCommandChange(nextDraft);
  };
    ...
    <section className="command-panel panel">
      ...
      <div className="suggestions">
        {suggestions.map((suggestion) => (
          <button key={suggestion} onClick={() => updateDraft(suggestion)}>
            {suggestion}
          </button>
        ))}
      </div>
      {(diagnosticDraftInspection.status !== 'none' || diagnosticDraftNotice) && (
        <div className={`command-diagnostic-draft ${diagnosticDraftInspection.status}`} aria-live="polite">
          <div>
              ...
              {diagnosticDraftInspection.status === 'complete'
                ...
                : diagnosticDraftInspection.status === 'ambiguous'
                  ...
                  : diagnosticDraftInspection.status === 'edited'
                    ? '检测到诊断块已被手工修改或只剩部分内容，为保护您的文字，不能自动移除。'
                    : diagnosticDraftNotice}
            </span>
          </div>
